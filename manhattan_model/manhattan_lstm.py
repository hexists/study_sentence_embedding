...
class Manhattan_LSTM(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        # dprint('hidden     = {}'.format(hidden.size()), color='red')

        outputs_1, hidden_1 = self.lstm_1(embedded_1, hidden)
        outputs_2, hidden_2 = self.lstm_2(embedded_2, hidden)

        # dprint('hidden_1[0] = {}'.format(hidden_1[0].size()), color='blue')
        # dprint('hidden_1[0].permute(1, 2, 0) = {}'.format(hidden_1[0].permute(1, 2, 0).size()), color='blue')
        # dprint('hidden_1[0].permute(1, 2, 0).view(batch_size, -1) = {}'.format(hidden_1[0].permute(1, 2, 0).view(batch_size, -1).size()), color='blue')
        # dprint('hidden_1[0] = {}'.format(hidden_1[0]), color='blue')
        # dprint('hidden_1[0].permute(1, 2, 0) = {}'.format(hidden_1[0].permute(1, 2, 0)), color='blue')
        # dprint('hidden_1[0].permute(1, 2, 0).view(batch_size, -1) = {}'.format(hidden_1[0].permute(1, 2, 0).view(batch_size, -1)), color='blue')
        ...
        # dprint('similarity_scores = {}'.format(similarity_scores), color='yellow')
