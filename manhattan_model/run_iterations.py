...
class Run_Iterations(object):
    def __init__(self, model_name, data_name, model, x_train, y_train, index2word, batch_size, num_iters,
    ...
    def train_iters(self):
        ...
        model_trainable_parameters = list(filter(lambda p: p.requires_grad, self.model.manhattan_model.parameters()))
        ...
        for epoch in range(1, self.num_iters + 1):
            for i in range(0, self.train_samples, self.batch_size):
                ...
                nn.utils.clip_grad_norm_(self.model.manhattan_model.parameters(), 5)
