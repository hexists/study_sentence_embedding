...
from manhattan_cnn import Manhattan_CNN
...
if __name__ == "__main__":
    ...
    parser.add_argument("-mn", "--model_name", type=str, help="Model name.", default="lstm")
    ...
    parser.add_argument("-e", "--embd_file", type=str, help="Path to Embedding File.", default="./GoogleNews-vectors-negative300.bin")
    ...
    if args.model_name == 'cnn':
        model = Manhattan_CNN(args.data_name, 300, embedding.embedding_matrix, use_embedding=True, train_embedding=True)
    else:
    ...
    train_network = Train_Network(args.model_name, model, data.index2word)

    run_iterations = Run_Iterations(args.model_name, args.data_name, train_network, data.x_train, data.y_train, data.index2word,
    ...
    torch.save(model.state_dict(), './manhattan_{}.pt'.format(args.model_name))
